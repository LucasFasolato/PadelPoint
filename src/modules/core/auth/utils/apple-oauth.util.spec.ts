...
function makeConfig(overrides: Record<string, string | undefined> = {}): { get: (key: string) => string | undefined } {
...
describe('isAppleOAuthEnabled', () => {
  ...
  it('returns false when APPLE_CLIENT_ID is missing', () => {
    expect(isAppleOAuthEnabled(makeConfig({ APPLE_CLIENT_ID: undefined }))).toBe(false);
  });

  it('returns false when APPLE_TEAM_ID is missing', () => {
    expect(isAppleOAuthEnabled(makeConfig({ APPLE_TEAM_ID: undefined }))).toBe(false);
  });

  it('returns false when APPLE_KEY_ID is missing', () => {
    expect(isAppleOAuthEnabled(makeConfig({ APPLE_KEY_ID: undefined }))).toBe(false);
  });

  it('returns false when APPLE_PRIVATE_KEY is missing', () => {
    expect(isAppleOAuthEnabled(makeConfig({ APPLE_PRIVATE_KEY: undefined }))).toBe(false);
  });

  it('returns false when APPLE_CALLBACK_URL is missing', () => {
    expect(isAppleOAuthEnabled(makeConfig({ APPLE_CALLBACK_URL: undefined }))).toBe(false);
  });

  it('returns false when a var is an empty string', () => {
    expect(isAppleOAuthEnabled(makeConfig({ APPLE_CLIENT_ID: '' }))).toBe(false);
  });

  it('returns false when a var is whitespace only', () => {
    expect(isAppleOAuthEnabled(makeConfig({ APPLE_TEAM_ID: '   ' }))).toBe(false);
  });

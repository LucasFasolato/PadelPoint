...
export function parseScoreSummary(summary: string | null | undefined): SetDto[] {
  ...
  for (const token of normalized.matchAll(SET_PATTERN)) {
    ...
    const tb1 =
      tb1Raw != null ? Number.parseInt(tb1Raw, 10) : Number.NaN;
    const tb2 =
      tb2Raw != null ? Number.parseInt(tb2Raw, 10) : Number.NaN;
...
export function buildScoreSummary(sets: Array<Pick<SetDto, 'a' | 'b'>>): string {
